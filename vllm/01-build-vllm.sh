...
# Step 3: Install build dependencies from README.md
echo ""
...
# Step 4: Run use_existing_torch.py
echo ""
...
# Step 5: Install ROCm build requirements
echo ""
echo "=== Installing ROCm Build Requirements ==="
# Remove amdsmi from rocm-build.txt to avoid segfaults on gfx1151
/usr/bin/sed -i '/amdsmi==/d' vllm/requirements/rocm-build.txt
pip install -r vllm/requirements/rocm-build.txt

# Step 6: Set PyTorch ROCm architecture
echo ""
echo "=== Setting PyTorch ROCm Architecture ==="
export PYTORCH_ROCM_ARCH="gfx1151"

# Step 7: Apply fixes for gfx1151 support
echo ""
...
if ! /usr/bin/grep -q "gfx1151" vllm/CMakeLists.txt; then
    /usr/bin/sed -i 's/set(HIP_SUPPORTED_ARCHS "gfx906;gfx908;gfx90a;gfx942;gfx950;gfx1030;gfx1100;gfx1101;gfx1200;gfx1201")/set(HIP_SUPPORTED_ARCHS "gfx906;gfx908;gfx90a;gfx942;gfx950;gfx1030;gfx1100;gfx1101;gfx1151;gfx1200;gfx1201")/' vllm/CMakeLists.txt
...
if /usr/bin/grep -q "torch == 2.8.0," vllm/pyproject.toml; then
    /usr/bin/sed -i '/torch == 2.8.0,/d' vllm/pyproject.toml
...
if ! /usr/bin/grep -q "TORCH_AVAILABLE = False" vllm/setup.py; then
    # Apply the torch import fix
    /usr/bin/sed -i 's/import torch/try:\n    import torch\n    from torch.utils.cpp_extension import CUDA_HOME, ROCM_HOME\n    TORCH_AVAILABLE = True\nexcept ImportError:\n    torch = None\n    CUDA_HOME = None\n    ROCM_HOME = None\n    TORCH_AVAILABLE = False/' vllm/setup.py
    
    # Fix other torch references
    /usr/bin/sed -i 's/from torch.utils.cpp_extension import CUDA_HOME, ROCM_HOME/# Moved to try block above/' vllm/setup.py
    /usr/bin/sed -i 's/torch.version.cuda is None/TORCH_AVAILABLE and torch.version.cuda is None/' vllm/setup.py
    /usr/bin/sed -i 's/has_cuda = torch.version.cuda is not None/has_cuda = TORCH_AVAILABLE and torch.version.cuda is not None/' vllm/setup.py
    /usr/bin/sed -i 's/torch.version.hip is not None/TORCH_AVAILABLE and torch.version.hip is not None/' vllm/setup.py
    /usr/bin/sed -i 's/rocm_version = get_rocm_version() or torch.version.hip/rocm_version = get_rocm_version() or (torch.version.hip if TORCH_AVAILABLE else None)/' vllm/setup.py
    /usr/bin/sed -i 's/cuda_major, cuda_minor = torch.version.cuda.split(".")/cuda_major, cuda_minor = torch.version.cuda.split(".") if TORCH_AVAILABLE else ("0", "0")/' vllm/setup.py
...
if ! /usr/bin/grep -q "Skip amdsmi check due to segfault issues" vllm/vllm/platforms/__init__.py; then
    # Replace the amdsmi detection with torch-ba/usr/bin/sed detection
    /usr/bin/sed -i '/def rocm_platform_plugin/,/return "vllm.platforms.rocm.RocmPlatform" if is_rocm else None/{
...
# Step 8: Build vLLM with ROCm target device
echo ""
echo "=== Building vLLM ==="
echo "Uninstalling amdsmi to prevent segfaults during build..."
pip uninstall amdsmi -y || echo "⚠ amdsmi not installed, continuing..."
